...
and *median* close to **`r round(daily.mean/1e3)` thousand steps** taken daily.
...
(interval `r peak.row$interval`) of `r round(peak.row$x)` steps on average,
...
The dataset contains `r nrow(na.values)` missing values, which makes up
`r round(100*nrow(na.values)/nrow(data))`%. As we can see from the distribution
